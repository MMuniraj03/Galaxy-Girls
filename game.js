(() => {
  ...
  function createEnemy() {
    ...
    // Spawn enemies directly in the heroine's firing lane. This keeps the
    // enemy body aligned with the bullet instead of spawning underneath it.
    const bulletLaneY = state.playerY + player.offsetHeight * 0.48;
    const y = clamp(
      bulletLaneY - el.offsetHeight * 0.45,
      70,
  ...
  function updateEnemies(dt) {
    ...
    for (let i = state.enemies.length - 1; i >= 0; i--) {
      ...
      const bulletLaneY = state.playerY + player.offsetHeight * 0.48;
      const targetY = bulletLaneY - enemy.el.offsetHeight * 0.45;
      // Smoothly follow the heroine's bullet lane (including when she jumps).
      enemy.y += (targetY - enemy.y) * Math.min(1, 7 * dt);
