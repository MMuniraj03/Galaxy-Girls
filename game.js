(() => {
  ...
  /* ==========================================================
     AUDIO — lightweight Web Audio effects, no external files needed
     ========================================================== */
  ...
  function startAmbientSound() {
    if (!audioCtx || !masterGain || !soundEnabled || ambientNodes.length) return;

    // Very quiet space-station ambience: two low oscillators with slow fades.
    const now = audioCtx.currentTime;
    const frequencies = [55, 82.5];
    frequencies.forEach((frequency, index) => {
      ...
      gain.gain.linearRampToValueAtTime(index === 0 ? 0.012 : 0.007, now + 2.5);
      osc.connect(gain);
  ...
  function tone(frequency, duration, type = "sine", volume = .06, slideTo = null, delay = 0) {
    if (!soundEnabled) return;
    initAudio();
    ...
    gain.gain.linearRampToValueAtTime(volume, start + Math.min(.008, duration * .15));
  ...
  function playShootSound() {
    tone(180, .07, "sawtooth", .12, 65);
    tone(900, .035, "square", .035, 300, .006);
  }

  function playJumpSound() {
    tone(360, .13, "square", .065, 820);
    tone(720, .08, "sine", .035, 1050, .025);
  }

  function playHitSound() {
    tone(270, .055, "square", .035, 120);
  }

  function playEnemyDeathSound() {
    tone(230, .12, "sawtooth", .075, 55);
    tone(100, .09, "square", .035, 45, .035);
  }

  function playBossDeathSound() {
    tone(95, .45, "sawtooth", .11, 35);
    tone(180, .32, "square", .07, 55, .08);
    tone(520, .5, "triangle", .055, 90, .18);
    tone(880, .32, "sine", .045, 260, .3);
  }

  function playBossSpawnSound() {
    tone(110, .22, "sawtooth", .09, 300);
    tone(300, .35, "square", .06, 75, .13);
  }
  ...
  function toggleSound() {
    ...
    if (soundEnabled) {
      initAudio();
      tone(660, .08, "sine", .045, 900);
    } else {
      stopAmbientSound();
      if (masterGain && audioCtx) masterGain.gain.setTargetAtTime(0, audioCtx.currentTime, .025);
    }
